  ...
  render() {
    ...
    if (translucent) {
      if(line) {
        if (Platform.OS === 'ios') {
          return (
            <LinearGradient
              ...
              style = {{ width: width, height: deviceInfo.isIphoneX ? 44 : 20,zIndex:1,position:'absolute'}}>
        ...
        if (Platform.OS === 'ios') {
          return (
            <View style = {{ width: width, height: deviceInfo.isIphoneX ? 44 : 20,zIndex:1,position:'absolute',backgroundColor: backgroundColor}}>
              <StatusBar barStyle={barStyle} hidden={hidden}/>
            </View>
          )
        } else if (Platform.OS === 'android') {
      ...
      if(line) {
        ...
        if (Platform.OS === 'ios') {
          return (
            <View style = {{ height: deviceInfo.isIphoneX ? 44 : 20,backgroundColor: backgroundColor}}>
              <StatusBar barStyle={barStyle} hidden={hidden}/>
            </View>
          )
        } else if (Platform.OS === 'android') {
