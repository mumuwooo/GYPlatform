...
export function checkPhoneNum(value){// 验证手机规则
...
export const sortQuestion = (a,b)=>  a.TypeInfo.QuestionType_ID - b.TypeInfo.QuestionType_ID
...
export const formatQuestion2Type = data=>{
  ...
  for (const i in data) {
    ...
    if (data[i].QuestionType_ID == 2) {
      data[i].Judge = data[i].MyAnswer ? data[i].MyAnswer == data[i].Answer.split(',').sort().toString(): 0
    }else if(data[i].QuestionType_ID == 3||data[i].QuestionType_ID == 5){
      data[i].Judge = null
    }else{
      data[i].Judge = data[i].MyAnswer ? data[i].MyAnswer == data[i].Answer: 0
    }
    if(data[i].MyAnswer||data[i].FileList){// 是否已答题
      data[i]._did = true
    }
       ...
       const qItem = questions[j]
        ...
        if (qItem.TypeInfo.QuestionType_ID == data[i].QuestionType_ID){
          ...
          if (j >0) {
            ...
            data[i]._no = questions[j - 1].Question[prevLen - 1]._no + qItem.Question.length + 1
          }else{
            // 当前类型为第一个
...
export function QuestionTypeName(num) {// 题目序号
  switch (num) {
    case 1:
      return '单选题'
    case 2:
...
// 遍历获取试题数据
export function  getQuestionCount (questions,modeType='exam',total=0,doCount=0,rightCount=0,sectionTotal=0,wrongCount=0,subjectiveCount=0,score=0){
  // hasSubjective 主观题
      for(const j in questions){
        const qItem = questions[j]
        total+=1;
        if(qItem.QuestionType_ID==3||qItem.QuestionType_ID==5)
          sectionTotal+=1
        if( qItem.FileList||qItem._did||
          (qItem.MyAnswer&&(modeType==='analysis'||modeType==='exam'||modeType=='chaptexer')&&qItem.MyAnswer)
          ...
          if(qItem.QuestionType_ID==5||qItem.QuestionType_ID==3){
            ...
            // 根据题型判断正误
            if(qItem.QuestionType_ID==1||qItem.QuestionType_ID==4||(modeType!='chaptexer'||qItem._submit)){
              wrongCount+=1;
            }
          }
        }
        if(qItem.Judge===1){
          // 根据题型判断正误
          if(qItem.QuestionType_ID==1||qItem.QuestionType_ID==2||qItem.QuestionType_ID==4||(modeType!='chaptexer'||qItem._submit)){
            rightCount+=1
  ...
  return {sectionTotal,total,doCount,rightCount,wrongCount,subjectiveCount,score}
}
// 时间转换{h,m,s}
...
export function getfileType(path){// 获取文件类型
  const tailArr = path.split(".")
  const tail = tailArr[tailArr.length-1]
  if(tail.search(/^(jpg|png|gif|jpeg|bmp)$/i)>-1) return {mode:"img",mimeType:`${tail.toLowerCase()}/image`}
  if(tail.search(/^txt$/i)>-1) return {mode:'txt',mimeType:`txt/plain`}
  if(tail.search(/^doc$/i)>-1) return {mode:'doc',mimeType:`application/msword`}
  if(tail.search(/^docx$/i)>-1) return {mode:'doc',mimeType:`application/vnd.openxmlformats-officedocument.wordprocessingml.document`}
  if(tail.search(/^xls$/i)>-1) return {mode:'xls',mimeType:`application/vnd.ms-excel`}
  if(tail.search(/^xlsx$/i)>-1) return {mode:'xls',mimeType:`application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`}
  if(tail.search(/^ppt$/i)>-1) return {mode:'ppt',mimeType:`application/vnd.ms-powerpoint`}
  if(tail.search(/^pptx$/i)>-1) return {mode:'ppt',mimeType:`application/vnd.openxmlformats-officedocument.presentationml.presentation`}
  if(tail.search(/^pdf$/i)>-1) return {mode:'pdf',mimeType:`application/pdf`}
  if(tail.search(/^zip$/i)>-1) return {mode:'zip',mimeType:`application/zip`}
